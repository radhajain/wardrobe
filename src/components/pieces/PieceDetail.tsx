...
import { ClothesWithId, Clothes, Outfit, ClothingType, ClothingTypes } from '../../types';
...
export function PieceDetail({
	...
	const getOutfitThumbnail = (outfit: Outfit) => {
		...
		return thumbnailItems.map((item) => {
							...
							{editingTag === 'type' ? (
								<div className="piece-detail__tag-edit">
									<select
										...
										value={editedType}
										onChange={(e) => setEditedType(e.target.value as ClothingType)}
										autoFocus
									>
